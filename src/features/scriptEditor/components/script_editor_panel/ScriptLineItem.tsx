    ...
    <div className={`p-3 mb-2 rounded-lg border flex items-center gap-3 transition-all duration-150 ${isSilentLine ? 'border-slate-800 opacity-70' : 'border-slate-700'} hover:border-slate-600 ${line.isAiAudioLoading ? 'opacity-70' : ''} ${isShortcutActive ? 'ring-2 ring-amber-400' : ''}`}>
      ...
      <div className="relative flex-shrink-0" style={{width: '6rem'}} ref={soundTypeDropdownRef}>
        <div className="flex w-full h-9 rounded-md border border-slate-600 overflow-hidden">
          ...
          <button onClick={() => setIsSoundTypeDropdownOpen(prev => !prev)} className={`flex-shrink-0 h-full flex items-center justify-center px-1.5 transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500 focus:z-10 border-l ${isLit ? 'bg-orange-500 hover:bg-orange-600 text-white border-orange-600' : 'bg-slate-700 hover:bg-slate-600 text-slate-300 border-slate-600'}`} aria-label="选择音效类型">
            ...
            <ul className="py-1">
              {soundOptions.map(option => {
                ...
                const handleOptionClick = () => {
                  ...
                  else { onUpdateSoundType(line.id, option === '清除' ? '' : option); setIsSoundTypeDropdownOpen(false); }
                };
                const handleDeleteClick = (e: React.MouseEvent) => {
                  e.stopPropagation();
                  if (window.confirm(`确定要删除自定义音效 "${option}" 吗？所有使用此音效的行将被重置。`)) onDeleteCustomSoundType(option);
                };
                return (
                  <li key={option} onClick={handleOptionClick} className={`flex items-center justify-between px-3 py-2 text-sm cursor-pointer group ${isSelected ? 'bg-sky-600 text-white' : 'text-slate-200 hover:bg-slate-700'}`}>
                    <span>{option}</span>
                    {isCustom && (<button onClick={handleDeleteClick} className="p-1 -mr-2 rounded-full text-slate-500 group-hover:text-red-400 hover:bg-slate-600" title={`删除 "${option}"`}><XMarkIcon className="w-4 h-4" /></button>)}
                  </li>
                );
              })}
            </ul>
