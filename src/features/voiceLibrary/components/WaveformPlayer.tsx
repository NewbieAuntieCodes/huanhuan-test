...
const WaveformPlayer: React.FC<WaveformPlayerProps> = ({ audioUrl, children, audioContext, isActive, onActivate }) => {
  ...
  useEffect(() => {
    if (!isActive && audioRef.current) {
      ...
      // Detach source to free up memory and network resources
  ...
  useEffect(() => {
    if (audioBuffer) {
      ...
      drawWaveform(null, 0); // Clear canvas if buffer is removed
    }
  ...
  return (
    <div className="w-full flex items-center gap-2 bg-slate-800 p-1.5 rounded-md">
       <audio
        ref={audioRef}
        onLoadedMetadata={() => {
        ...
        preload="metadata"
      />
      <button onClick={handlePlayPause} disabled={isLoading || (!isActive && !audioUrl)} className="p-2 bg-slate-700 hover:bg-sky-500 rounded-full disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="flex-grow h-10 relative">
        ...
        <canvas ref={canvasRef} onClick={handleCanvasClick} className={`w-full h-full ${isLoading || error ? 'opacity-20' : ''}`} />
      </div>
